...
class SphereMesh:
    ...
    def get_octant_axes_and_elements_count(self, octant_shape):
        ...
        if octant_number in [2, 4, 5, 7]:
            ...
            axes = [scale_vector(axesV2[c], axesSignsV2[c]) for c in range(3)]
        else:
            boxDerivatives = [boxDerivativesV1[c] * signs[c] for c in range(3)]
            elementsCountAcross = elementsCountAcrossV1
            axes = [scale_vector(axesV1[c], axesSignsV1[c]) for c in range(3)]
...
class OctantMesh:
    ...
    def sample_curves_between_two_nodes_on_sphere(self, id1, id2, elementsOut, dStart, dbetween, dEnd):
        ...
        for ni in range(elementsCount + 1):
            ...
            if ni < len(dStart):
                if dStart[ni]:
                    btd[dStart[ni]][idi[0]][idi[1]][idi[2]] = nd1[nit] if dStart[ni] > 0 else scale_vector(
                        nd1[nit], -1)
                    nit += 1
            elif ni > elementsCount - len(dEnd):
                nie = ni - elementsCount + len(dEnd) - 1
                if dEnd[nie]:
                    btd[abs(dEnd[nie])][idi[0]][idi[1]][idi[2]] = nd1[nit] if dEnd[nie] > 0 else scale_vector(
                        nd1[nit], -1)
                    nit += 1
            else:
                ...
                btd[abs(dbetween[0])][idi[0]][idi[1]][idi[2]] = nd1[nit] if dbetween[0] > 0 else scale_vector(
                    nd1[nit], -1)
                nit += 1
                ...
                if ni < len(dStart[se]):
                    if dStart[se][ni]:
                        ...
                        if dStart[0][ni] > 0:
                            ...
                            td.append(scale_vector(btd[abs(dStart[se][ni])][ids[0]][ids[1]][ids[2]], -1))
                    ...
                    if dEnd[se][nie]:
                        ...
                        if dEnd[se][nie] > 0:
                            ...
                            td.append(scale_vector(btd[abs(dEnd[se][nie])][ids[0]][ids[1]][ids[2]], -1))
                else:
                    tx.append(btx[ids[0]][ids[1]][ids[2]])
                    if dBetween[se][0] > 0:
                        td.append(btd[abs(dBetween[se][0])][ids[0]][ids[1]][ids[2]])
                    else:
                        td.append(scale_vector(btd[abs(dBetween[se][0])][ids[0]][ids[1]][ids[2]], -1))
                ...
                if ni < len(dStart[se]):
                    if dStart[se][ni]:
                        if dStart[0][ni] > 0:
                            ...
                            btd[abs(dStart[se][ni])][ids[0]][ids[1]][ids[2]] = scale_vector(td[nit], -1)
                        nit += 1
                elif ni > elementsCount[se] - len(dEnd[se]):
                    nie = ni - elementsCount[se] + len(dEnd[se]) - 1
                    if dEnd[se][nie]:
                        if dEnd[se][nie] > 0:
                            ...
                            btd[abs(dEnd[se][nie])][ids[0]][ids[1]][ids[2]] = scale_vector(td[nit], -1)
                        nit += 1
                else:
                    if dBetween[se][0] > 0:
                        btd[abs(dBetween[se][0])][ids[0]][ids[1]][ids[2]] = td[nit]
                    else:
                        btd[abs(dBetween[se][0])][ids[0]][ids[1]][ids[2]] = scale_vector(td[nit], -1)
                    nit += 1
    ...
    def calculate_interior_quadruple_point(self):
        ...
        radius = magnitude(ra)
        local_x = scale_vector(ra, (1 - ts/radius))
