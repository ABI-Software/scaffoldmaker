...
from scaffoldmaker.utils.interpolation import smoothCubicHermiteDerivativesLine, sampleCubicHermiteCurves, \
    interpolateSampleCubicHermite, DerivativeScalingMode
...
class CapMesh:
    ...
    def __init__(self, elementsCountAround, elementsCountCoreBoxMajor, elementsCountCoreBoxMinor,
        ...
        # list[startCap, endCap][[x, d1, d2, d3][nThroughWall][nAcrossMajor][nAcrossMinor].
        self._startCapNodeIds = None
        # capNodeIds that form the cap at the start of a tube segment.
        # list[nThroughWall][nAcrossMajor][nAcrossMinor].
    ...
    def _sampleCapCoordinates(self, s):
        """
        ...
        def _getRatioBetweenTwoRadii(rList, oList):
            """
            Calculates the ratio between the original radius of a sphere and the new radius of an ellipsoid.
            :param rList: List of new radius in each direction.
            :param oList: List of original radius in each direction.
            :return: List of ratio between two radii in x, y, and z-direction.
            """
            return [rList[c] / oList[c] for c in range(3)]
        ...
        for n3 in range(nodesCountRim):
            ...
            ratio = _getRatioBetweenTwoRadii(radii, oRadii)
    ...
    def _calculateShellRegularNodeCoordinates(self, n3, centre):
        ...
        for m in range(self._elementsCountCoreBoxMajor + 1):
            for n in [0, -1]:
                ...
                if x1 is None:
                    continue
                nx, nd1 = (sampleCurvesOnSphere(x1, x2, centre, elementsOut) if n == 0 else
                           sampleCurvesOnSphere(x2, x1, centre, elementsOut))
                start, end = (
        ...
        for n in range(self._elementsCountCoreBoxMinor + 1):
            for m in [0, -1]:
                ...
                if x1 is None:
                    continue
                nx, nd2 = (sampleCurvesOnSphere(x1, x2, centre, elementsOut) if m == 0 else
                           sampleCurvesOnSphere(x2, x1, centre, elementsOut))
                start, end = (
    ...
    def _sphereToSpheroid(self, n3, ratio, centre):
        ...
        mCount = self._elementsCountCoreBoxMajor + 1
        nCount = self._elementsCountCoreBoxMinor + 1
    ...
    def _calculateBoxMajorAndMinorNodes(self):
        ...
        for m in [0, -1]:
            ...
            tx, td3, pe, pxi, psf = sampleCubicHermiteCurves(nx, nd3, self._elementsCountCoreBoxMinor,
    ...
    def _getNodesCountRim(self):
        return self._elementsCountThroughShell + self._elementsCountTransition

    def _getElementsCountRim(self):
        elementsCountRim = max(1, self._elementsCountThroughShell) + self._elementsCountTransition
        return elementsCountRim

    def _generateNodes(self):
        """
        Blackbox function for generating cap nodes.
        """
    ...
    def _generateExtendedTubeNodes(self):
        ...
        for n3 in range(nodesCountRim):
            ...
            rx, rd1, rd2, rd3 = [
                tx[i][n3 if elementsCountTransition > 1 and n3 < (elementsCountTransition - 1) else n3p] for i in
                range(4)]
    ...
    def _generateElements(self, annotationMeshGroups):
        """
                ...
                for e1 in range(elementsCountCoreBoxMinor):
                    ...
                    for n1 in [e1, e1 + 1]:
                        ...
                        for n3 in [e3, e3p]:
                            ...
                            nodeLayoutCapBoxShieldTriplePoint = generateData.getNodeLayoutCapBoxShieldTriplePoint(
                                tpLocation, isStartCap)
    ...
    def _generateExtendedTubeElements(self, tubeBoxNodeIds, tubeRimNodeIds, annotationMeshGroups):
        ...
        elementsCountRim = self._getElementsCountRim()
        elementsCountRimRegular = elementsCountRim - 1
    ...
    def generateElements(self, tubeBoxNodeIds, tubeRimNodeIds, annotationMeshGroups, isStartCap=True):
        """
    ...
    def addBoxElementsToMeshGroup(self, meshGroup, e1Range=None, e2Range=None, e3Range=None):
        ...
        for i, isCap in enumerate(self._isCap):
            ...
            addElementsFromIdentifiers(mesh, meshGroup, capElementIds[0], tRange=range(elementsCountTransition + 1), mRange=e1Range, nRange=e3Range)

            # Cap tube elements (t, e2)
            addElementsFromIdentifiers(mesh, meshGroup, capElementIds[1], tRange=range(elementsCountTransition), e2Range=e2Range)

            # Extension base block (just one layer at t=0)
            addElementsFromIdentifiers(mesh, meshGroup, extElementIds[0][0:1], tRange=[0], mRange=e1Range, nRange=e3Range)
    ...
    def addShellElementsToMeshGroup(self, meshGroup, e1Range=None, e2Range=None, e3Range=None):
        ...
        for i, isCap in enumerate(self._isCap):
            ...
            addElementsFromIdentifiers(mesh, meshGroup, capElementIds[0], tRange=tCapShellRange, mRange=e1Range, nRange=e3Range)

            tTubeShellRange = range(elementsCountTransition, elementsCountTransition + elementsCountThroughShell)
            addElementsFromIdentifiers(mesh, meshGroup, capElementIds[1], tRange=tTubeShellRange, e2Range=e2Range)
...
def sampleCurvesOnSphere(x1, x2, origin, elementsOut):
    ...
    :param x1: Coordinates of point 1 on the spherical surface of cap mesh.
    :param x2: Coordinates of point 2 on the spherical surface of cap mesh.
...
def addElementsFromIdentifiers(mesh, meshGroup, identifiers, tRange=None, mRange=None, nRange=None, e2Range=None):
    """
