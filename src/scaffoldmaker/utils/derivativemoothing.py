...
class EdgeCurve:
    ...
    def getExpression(self, expressionIndex):
        """
        Caller must have called evaluateArcLength!
        :param expressionIndex: 0 = start x, 1 = start d, 2 = end x, 3 = end d
        """
...
class DerivativeSmoothing:
    ...
    def __init__(self, region, field, selectionGroupName=None, scalingMode=DerivativeScalingMode.ARITHMETIC_MEAN,
                 editGroupName=None):
        """
        :param field: Finite element field to smooth.
    ...
    def addElementEdges(self):
        ...
        while element.isValid():
            ...
            if not eft.isValid():
                ...
                if eft1.isValid():
                    print('DerivativeSmoothing: Skip element', element.getIdentifier(),
                          'with per-component element field, which is not implemented')
                # else:
                #     print('DerivativeSmoothing: Skip element', element.getIdentifier(),
                #           'as field is not defined on it')
                useElement = False
                ...
                for n in range(1, localNodesCount + 1):
                    ...
                    # print('DerivativeSmoothing: Skip element', element.getIdentifier(),
                    #       'as it contains no selected nodes')
                    useElement = False
            if useElement:
                ...
                for n in range(1, basisLocalNodeCount):
                    basisLocalNodeFunctions.append(
                        basisLocalNodeFunctions[n - 1] + elementbasis.getNumberOfFunctionsPerNode(n))
                useDirections = []
                derivativeOffsets = []
                derivativeOffset = 1
                ...
                if len(useDirections) != dimension:
                    print('DerivativeSmoothing: Skip element', element.getIdentifier(),
                          'with unsupported basis function types', functionTypes)
                elif shapeType == Element.SHAPE_TYPE_CUBE:
                    for d in range(dimension):
                        if useDirections[d]:
                            for localNodes in self.cubeEdgeLocalNodes[d]:
                                self.addEdgeCurve(element, eft, basisLocalNodeFunctions, localNodes,
                                                  derivativeOffsets[d])
    ...
    def addEdgeCurve(self, element, eft, basisLocalNodeFunctions, basisLocalNodes, derivativeOffset):
        ...
        :param element: Element to add edge of.
        :param eft: Element field template.
        :param basisLocalNodeFunctions:
        ...
        """
        # print('element', element.getIdentifier(), 'fns', basisLocalNodeFunctions,
        #       'basisLocalNodes', basisLocalNodes, 'derivativeOffset', derivativeOffset)
        ...
        if existingEdgeCurve:
            # print('Found existing edge for global nodes', sortedNodeIdentifiers,
            #       'on element', element.getIdentifier(), 'basis nodes', basisLocalNodes)
            return
    ...
    def smooth(self, updateDirections=False, maxIterations=10, arcLengthTolerance=1.0E-6):
        """
        :param updateDirections: Set to True if directions are to be recalculated.
        ...
        with ChangeManager(self._fieldmodule):
            ...
            for smoothIter in range(maxIterations + 1):
                converged = True
                for edge in self._edgesMap.values():
                    ...
                    if (math.fabs(arcLength - lastArcLength)/arcLength) > arcLengthTolerance:
                        converged = False
                if converged:
                    print('Derivative smoothing: Converged after', smoothIter, 'iterations.')
                    break
                elif smoothIter == maxIterations:
