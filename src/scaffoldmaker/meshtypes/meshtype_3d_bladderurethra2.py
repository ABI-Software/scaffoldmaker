...
from scaffoldmaker.utils.geometry import createEllipsePoints, getApproximateEllipsePerimeter
...
class MeshType_3d_bladderurethra2(Scaffold_base):
    """
    ...
    @staticmethod
    def getName():
        return '3D Bladder with Urethra 2'
    ...
    def generateBaseMesh(cls, region, options):
        ...
        # Create the flat scaffold
        # Calculate the ellipses circumference along the bladder inner layer
        circumferenceList = []
        for n1 in range(1, elementsCountAlongBladder + 1):
            idMinor = (n1 - 1) * elementsCountAround * (elementsCountThroughWall + 1) + elementsCountThroughWall + 2
            minorNodeAlong = xFinal[idMinor - 1]
            if elementsCountAround % 4 == 0:
                idMajor = idMinor + elementsCountAround // 4
                MajorNodeAlong = xFinal[idMajor - 1]
            else:
                i = elementsCountAround // 4
                x1 = xFinal[idMinor + i - 1]
                x2 = xFinal[idMinor + i]
                for c in range(3):
                    x[c] = (x1[c] + x2[c]) / 2
                MajorNodeAlong = x
            radiusMinor = vector.magnitude(minorNodeAlong[c] - [0.0, 0.0, minorNodeAlong[2]][c] for c in range(3))
            radiusMajor = vector.magnitude(MajorNodeAlong[c] - [0.0, 0.0, MajorNodeAlong[2]][c] for c in range(3))
            circumference = getApproximateEllipsePerimeter(radiusMinor, radiusMajor)
            circumferenceList.append(circumference)

        # Obtain the flat nodes coordinates based on the circumference along the bladder
        xfList = []
        d1fList = []
        for n1 in range(elementsCountAround + 1):
            for n2 in range(1, elementsCountAlongBladder + 1):
                i = (n2 - 1) * elementsCountAround * (elementsCountThroughWall + 1) + elementsCountThroughWall + 2
                xfc = [circumferenceList[n2 - 1] / 2 - n1 * circumferenceList[n2 - 1] / elementsCountAround,
                       0.0,
                       xFinal[i][2]]
                # n2 * bladderLength / elementsCountAlongBladder
                d1fc = [-circumferenceList[n2 - 1] / elementsCountAround, 0.0, 0.0]
                xfList.append(xfc)
                d1fList.append(d1fc)

        # Add apex coordinates to the list
        # xApexLumen = [0.0, 0.0, 0.0]
        xFlat = []
        d1Flat = []
        d2Flat = []
        xFinalFlat = []
        d1FinalFlat = []
        d2FinalFlat = []
        d2ApexInner = [0.0, 0.0, 0.0]
        for n3 in range(elementsCountThroughWall + 1):
            xApex = [xApexInner[0],
                     xApexInner[1] + n3 * bladderWallThickness,
                     xApexInner[2]]
            xFlat.append(xApex)
            d1Flat.append(d1ApexInner)
            d2Flat.append(d2ApexInner)
        xFinalFlat += xFlat
        d1FinalFlat += d1Flat
        d2FinalFlat += d2Flat

        # Find the derivatives go down the bladder
        d2fList = []
        for n2 in range(elementsCountAround + 1):
            for n1 in range(0, elementsCountAlongBladder):
                if n1 == elementsCountAlongBladder - 1:
                    # if not includeUrethra:
                    d2 = [0.0, 0.0, bladderSegmentLength]
                else:
                    v1 = xfList[elementsCountAlongBladder * n2 + n1]
                    v2 = xfList[elementsCountAlongBladder * n2 + n1 + 1]
                    d2 = [v2[c] - v1[c] for c in range(3)]
                d2fList.append(d2)
        ...
        for n1 in range(elementsCountAround + 1):
            ...
            for n2 in range(elementsCountAlongBladder):
                lineSmoothingNodes.append(xfList[n1 * (elementsCountAlongBladder) + n2])
                lineSmoothingNodes_d2.append(d2fList[n1 * (elementsCountAlongBladder) + n2])
            smd2 = smoothCubicHermiteDerivativesLine(lineSmoothingNodes, lineSmoothingNodes_d2, fixAllDirections=True,
                                                     fixStartDerivative=True, fixEndDerivative=True,
                                                     fixStartDirection=False, fixEndDirection=False)
            smoothed_d2 += smd2
        ...
        for n3 in range(elementsCountThroughWall + 1):
            for n in range(len(xfList)):
                x = [xfList[n][0], n3 * bladderWallThickness, xfList[n][2]]
                d1 = d1fList[n]
                d2 = smoothed_d2[n]
                xFlat.append(x)
                d1Flat.append(d1)
        ...
        # Rearranage the nodes
        for n2 in range(1, elementsCountAlongBladder + 1):
            for n3 in range(elementsCountThroughWall + 1):
                for n1 in range(elementsCountAround + 1):
                    i = (elementsCountAround + 1) * elementsCountAlongBladder * n3 + n1 * elementsCountAlongBladder + n2 + elementsCountThroughWall
                    xFinalFlat.append(xFlat[i])
                    d1FinalFlat.append(d1Flat[i])
                    d2FinalFlat.append(d2Flat[i])
        ...
        nodeIdentifier, elementIdentifier, annotationGroups = createBladderNodesAndElements(
            region, xFinal, d1Final, d2Final, d3Final, xFlat, d1Flat, d2Flat, xOrgan, d1Organ, d2Organ, None,
            elementsCountAround, elementsCountAlong, elementsCountThroughWall,
            annotationGroupsAround, annotationGroupsAlong, annotationGroupsThroughWall,
            firstNodeIdentifier, firstElementIdentifier,
...
def createBladderNodesAndElements(region,
    x, d1, d2, d3,
    xFlat, d1Flat, d2Flat,
