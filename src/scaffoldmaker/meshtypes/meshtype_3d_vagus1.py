...
class MeshType_3d_vagus1(Scaffold_base):
    ...
    def defineFaceAnnotations(cls, region, options, annotationGroups):
        ...
        #     if 'trunk' in annotationGroup.getName():
        #         trunk_group = annotationGroup.getGroup()
        trunkAnnotationGroup = findAnnotationGroupByName(annotationGroups, "left vagus X nerve trunk")
        trunk_group = trunkAnnotationGroup.getGroup()

        for annotationGroup in annotationGroups:
            if 'branch' in annotationGroup.getName():
                branchNodesetGroup = annotationGroup.getNodesetGroup(nodes)
    ...
    def generateBaseMesh(cls, region, options):
        ...
        # remove trunk nodes from branch groups
        ...
        #     reset_nodes_with_fitted_nodes(fieldmodule, fitter, branch_name)
        fitter = fit_branches_model(fitter_model_file, fitter_data_file, None)
        reset_nodes_with_fitted_nodes(fieldmodule, fitter, None)
...
def reset_nodes_with_fitted_nodes(fieldmodule, fitter, group_name = None):
    """
    """
...
def load_data(region):
    ...
    trunk_data_coordinates = extract_group_nodeset_as_coordinates_list(fieldmodule, trunk_group_name)
    branch_data = {}
    for branch_name in branch_groups:
        branch_data_coordinates = extract_group_nodeset_as_coordinates_list(fieldmodule, branch_name)
...
def extract_group_nodeset_as_coordinates_list(fieldmodule, group_name):
    """
