"""
Generates a 3-D femoral head mesh with variable numbers of elements
...
class MeshType_3d_femoralhead1(Scaffold_base):
    '''
    classdocs
    '''
    @staticmethod
    def getName():
        return '3D Femoral head 1'

    @staticmethod
    def getDefaultOptions(parameterSetName='Default'):
        return {
            'Number of elements around' : 8,
            'Number of elements up' : 10,
            'Femoral head radius': 1.0,
            'Femoral neck radius': 0.5,
            'Wall thickness': 0.05,
            'Alpha factor': 0.5,
    ...
    def generateBaseMesh(cls, region, options):
        ...
        outerHeadRadius = options['Femoral head radius']
        outerNeckRadius = options['Femoral neck radius']
        ...
        radiansPerElementUp = 0.8 * math.pi / elementDataCloud  # default normal 0.8
        ...
        for n1 in range(elementDataCloud):
            ...
            for n2 in range(elementDataCloud):
                ...
                v = [ outerHeadRadius * cosRadiansAround * sinRadiansUp,
                      outerHeadRadius * sinRadiansAround * sinRadiansUp,
                     -outerHeadRadius * cosRadiansUp]
                xAlong.append(v)
            xList.append(xAlong)
        ...
        if camThicknessFactor > 0.0:
            ...
            radiusCamHead = (1 + camThicknessFactor) * outerHeadRadius
            radiansAroundCamList = [-radiansCamWidth, 0.0, radiansCamWidth]
            radiusHeadList = [outerHeadRadius, radiusCamHead, outerHeadRadius]
            elementCamStartUpIdx = int(elementDataCloud * alphaFactor)
            elementsDataAroundCam = int(camWidthFactor * elementDataCloud)

            for n2 in range(elementDataCloud - elementCamStartUpIdx):
                radiansUp = (elementCamStartUpIdx + n2) * radiansPerElementUp
                cosRadiansUp = math.cos(radiansUp)
                sinRadiansUp = math.sin(radiansUp)
                nx = []
                nd1 = []
                ...
                for n in range(-int(elementsDataAroundCam * 0.5 - 1), int(elementsDataAroundCam * 0.5)):
                    xList[n][elementCamStartUpIdx + n2] = sx[count]
                    count += 1
        ...
        # Create cylindrical neck
        for n1 in range(elementDataCloud):
            radiansAround = radiansPerElementAround * n1
            cosRadiansAround = math.cos(radiansAround)
            sinRadiansAround = math.sin(radiansAround)
            xAlong = []
            for n2 in range(int(elementDataCloud * 0.5)):
                v = [ outerNeckRadius * cosRadiansAround,
                      outerNeckRadius * sinRadiansAround,
                      outerHeadRadius + outerHeadRadius/(elementDataCloud *0.5) * n2]
                xAlong.append(v)
            xList[n1] += xAlong
        ...
        for n1 in range(elementDataCloud):
            nx = xList[n1]
            nd = dList[n1]
            sx, sd = interp.sampleCubicHermiteCurves(nx, nd, elementsCountUp)[:2]
        ...
        for n2 in range(1, elementsCountUp + 1):
            ...
            sx = interp.sampleCubicHermiteCurves(xAround, dAround, int(elementsCountAround * 0.5), arcLengthDerivatives=True)[0]
