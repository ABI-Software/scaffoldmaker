...
class MeshType_1d_nervecentreline2(Scaffold_base):
    ...
    def generateBaseMesh(cls, region, options):
        ...
        update_terms = True
        nerve_terms_changed = True
        filename = os.path.join(working_dir, 'coordinates_modified.json')
        coordinates_file = os.path.join(working_dir, 'coordinates_modified2.json')
        ...
        #
        # def get_nodes_coordinates(anatomical_names):
        #     filename = os.path.join(sys.argv[2], 'coordintes_modified.json')
        #     with open(filename, 'r') as f:
        #         data = json.loads(f.read())
        #
        #     nodes_data = []
        #     for key, values in data.items():
        #         anatomical_name = get_anatomical_name(key, anatomical_names)
        #         nodes_data.append(
        ...
        # if input_file:
        #     properties_data, anatomicalMap_data = read_manifest(input_file)

        # anatomical_names = get_anatomical_names(properties_data, anatomicalMap_data)
        # nerveMarkerPoints = get_nodes_coordinates(anatomical_names)
        ...
        #     for p in points:
        #         d[p] = {"x": (100*np.random.rand(3)).tolist(), "d1": [10.0, 10.0, 10.0], "r": 6.0}
        #
        ...
        #     markerName.assignString(cache, nerveMarkerPoints[n3]["name"])
        ...
        for name, centreline_values in centreline_data.items():
            ...
            try:
                ...
                pass
            el.append(centreline_values['connects'][1])
            centreline = []
...
def write_points_to_json(names_internal_anatomical_map, points, input_file=''):
    nodes_data = {}
