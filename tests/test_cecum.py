...
class CecumScaffoldTestCase(unittest.TestCase):
    ...
    def test_cecum_annotations(self):
        ...
        for term in cecum_terms:
            ...
            self.assertTrue(("UBERON" in upper_id) or ("ILX" in upper_id) or (upper_id == ""), "Invalid cecum term" + str(term))
            if len(term) > 2:
                ...
                self.assertTrue(fma_index > uberon_index and fma_index > ilx_index, 'FMA term should be written after UBERON and ILX terms on cecum term ' + str(term))
