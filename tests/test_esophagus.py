...
class EsophagusScaffoldTestCase(unittest.TestCase):
    ...
    def test_esophagus_annotations(self):
        ...
        for term in esophagus_terms:
            ...
            self.assertTrue(("UBERON" in upper_id) or ("ILX" in upper_id) or (upper_id == ""), "Invalid esophagus term" + str(term))
            if len(term) > 2:
                ...
                self.assertTrue(fma_index > uberon_index and fma_index > ilx_index, 'FMA term should be written after UBERON and ILX terms on esophagus term ' + str(term))
