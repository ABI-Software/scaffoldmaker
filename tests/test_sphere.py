...
class SphereScaffoldTestCase(unittest.TestCase):
    ...
    def test_sphere2_serendipity(self):
        """
        Test creation of Sphere 2 scaffold using serendipity.
        """
        scaffold = MeshType_3d_solidsphere2
        parameterSetNames = scaffold.getParameterSetNames()
        self.assertEqual(parameterSetNames, ["Default"])
        ...
        self.assertEqual([0, 0], options.get("Crop number of elements in direction 3"))
        self.assertEqual([1, 2, 3], options.get("Box derivatives"))
        self.assertFalse(options.get("Use serendipity"))
        options["Use serendipity"] = True
        ...
        result, surfaceArea = surfaceAreaField.evaluateReal(fieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(surfaceArea, 12.441185163093586, delta=tol)
        result, volume = volumeField.evaluateReal(fieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(volume, 4.122309120762601, delta=tol)
        ...
        result, surfaceArea = surfaceAreaField.evaluateReal(refineFieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(surfaceArea, 12.08104118999541, delta=tol)
        result, volume = volumeField.evaluateReal(refineFieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(volume, 3.8786130886491494, delta=tol)
        ...
        self.assertTrue(coordinates.isValid())
        minimums, maximums = evaluateFieldNodesetRange(coordinates, nodes)
        assertAlmostEqualList(self, minimums, [-0.5, -0.8, -1.0], tol)
        assertAlmostEqualList(self, maximums, [0.5, 0.8, 1.0], tol)
        ...
        result, surfaceArea = surfaceAreaField.evaluateReal(fieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(surfaceArea, 7.300630646267272, delta=tol)
        result, volume = volumeField.evaluateReal(fieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(volume, 1.6738223177203837, delta=tol)
