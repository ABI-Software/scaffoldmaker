...
class LungScaffoldTestCase(unittest.TestCase):
    ...
    def test_lung_annotations(self):
        """
        Test that all lung terms are UBERON or ILX. Empty terms are also accepted. FMA terms can be included, but should be listed after UBERON and ILX terms. 
        """
        for term in lung_terms:
            upper_id = term[1].upper()
            self.assertTrue(("UBERON" in upper_id) or ("ILX" in upper_id) or (upper_id == ""), "Invalid lung term" + str(term))
            if len(term) > 2:
                ...
                self.assertTrue(fma_index > uberon_index and fma_index > ilx_index, 'FMA term should be written after UBERON and ILX terms on lung term ' + str(term))
