...
class LungScaffoldTestCase(unittest.TestCase):
    ...
    def test_lung2(self):
        ...
        self.assertTrue(coordinates.isValid())
        minimums, maximums = evaluateFieldNodesetRange(coordinates, nodes)
        assertAlmostEqualList(self, minimums, [ -0.6380112948407249, -0.5136821588344925, -0.2594693243518892 ], 1.0E-6)
        assertAlmostEqualList(self, maximums, [ 0.6380112948407249, 0.36430478702255786, 0.9368327915434912 ], 1.0E-6)
        with ChangeManager(fieldmodule):
            one = fieldmodule.createFieldConstant(1.0)
            upperRightFissureGroup = fieldmodule.findFieldByName('upper lobe of right lung').castGroup()
            self.assertTrue(upperRightFissureGroup.isValid())
        ...
        result, surfaceArea = surfaceAreaField.evaluateReal(fieldcache, 1)
        self.assertEqual(result, RESULT_OK)
        self.assertAlmostEqual(surfaceArea, 2.145407946129267, delta=1.0E-2)
        result, volume = volumeField.evaluateReal(fieldcache, 1)
        self.assertEqual(result, RESULT_OK)
